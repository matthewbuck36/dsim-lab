# Offline physical snapshot agent instructions

This root contains physical TurtleBot source under `pi/ros2_ws/src`, even
though it is stored on the laptop. It is not the Gazebo workspace.
The local `pi/mbuck_backups/phase09_backups/offline_snapshot_history` and
`pi/mbuck_backups/sync_records` collections must survive snapshot refreshes.
