...
```bash
docs/DSIM_GESC_Gaussian_Codex_Implementation_Package/tools/validate_phase_context.sh 04 plan
```

If validation fails, stop and report the missing durable context.

# Phase 04 task: plan escape progress, assisted escape, and recentering

Read:
- `docs/DSIM_GESC_Gaussian_Codex_Implementation_Package/04_STATE_MACHINE_SPEC.md`
- `docs/DSIM_GESC_Gaussian_Codex_Implementation_Package/02_TARGET_ROS2_ARCHITECTURE.md`

Plan:
- radial progress measurement,
- fixed escape center/radius,
- stall detection,
- one fill escalation before assisted escape,
- safe affine direction,
- wall/bounds checks,
- direction candidate scoring,
- recenter behavior,
- planner reuse decision,
- command arbitration and saturation,
- timeout/failsafe,
- simulation-only tests.

Apply the deterministic rule:
- reuse an already integrated planner if it exists,
- otherwise implement a bounded center-return controller in the existing control package.
...
enough to save without relying on the chat history. Before opening the Phase 04
