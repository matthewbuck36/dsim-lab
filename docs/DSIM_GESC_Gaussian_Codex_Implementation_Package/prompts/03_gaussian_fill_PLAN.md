...
```bash
docs/DSIM_GESC_Gaussian_Codex_Implementation_Package/tools/validate_phase_context.sh 03 plan
```

If validation fails, stop and report the missing durable context.

# Phase 03 task: plan robust basin estimation and Gaussian fill redesign

Read:
- `docs/DSIM_GESC_Gaussian_Codex_Implementation_Package/03_ROBUST_GAUSSIAN_ALGORITHM_SPEC.md`

Plan exact implementation of:
- synchronized recent sample buffer,
- outlier rejection,
- kernel/cost-weighted center estimate,
- covariance,
- regularized quadratic fit,
- depth/curvature amplitude,
- width scaling,
- local-grid residual-minimum check,
- escalation,
- soft association plus hard merge gate,
- fill registry revisions/supersession,
- confidence and diagnostics.

The plan must identify which current Gaussian code will be extended or refactored and how existing wiring remains intact.

Include deterministic unit-test fields and expected numerical outcomes.
...
enough to save without relying on the chat history. Before opening the Phase 03
