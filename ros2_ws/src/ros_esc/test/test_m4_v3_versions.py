"""V3 group-shutdown version separation: no simulation, bags or field evaluation."""
...
BEFORE=Path('/home/mattb/Experiments/GESC-Gaussian/v2/builds/m4_v3_recorder_shutdown_v1/version_before')
...
workflow=module('m4_v3_workflow_fixture',TOOLS/'m4_workflow.py')
dispatch=module('m4_v3_dispatch_fixture',TOOLS/'run_m4.py')
evaluator=module('m4_v3_evaluator_fixture',TOOLS/'evaluate_m4.py')
...
def contract(tmp_path,monkeypatch):
    ...
    version='m4-pilot-v3'; identity=scenario.experiment_identity(version)
    ...
    record=dict(schema_version=1,version=version,experiment_version=version,method_version=metrics.VERSION,
        ...
        execution=dict(runs_root=str(root/'runs'),process_ownership_mode='subreaper_group_v3',
...
def test_fresh_version_preserves_every_scientific_setting_and_all_caps(contract):
    ...
    v3=scenario.build_m4_document(topology(),runs_root=root/'runs',experiment_version=version)
    v1=scenario.build_m4_document(topology(),runs_root=root/'runs')
    assert v3['suite_id']=='m4_pilot_v3' and v3['metadata']['experiment_version']==version
    for first,second in zip(v1['cases'],v3['cases']):
        assert second['case_id'].startswith('m4_v3_')
    ...
    assert all('--process-ownership-mode' in row['runner_argv'] and 'subreaper_group_v3' in row['runner_argv'] for row in contract['runs'])
...
def test_v3_contract_rejects_old_identity_or_changed_method(contract,fault):
    row=contract['runs'][0]
...
def test_v3_resource_cannot_write_old_root_and_preserves_existing_files(contract):
...
def test_analysis_identity_is_separate_from_unchanged_method(contract):
    ...
    assert normalized['experiment_version']=='m4-pilot-v3'
...
def test_v3_all_slot_aggregation_retains_192_targets_and_rejects_v1_row(contract):
    ...
    assert all(r['run_id'].startswith('m4-pilot-v3-') for r in result['direction_rows'])
...
def test_v3_process_proof_cannot_be_replaced_by_old_empty_cleanup(contract):
    ...
    result={'process_ownership':{'mode':'subreaper_group_v3','signal_strategy':'initial_unreaped_root_group_sigint_then_adopted_pidfd','inspection_complete':True,'kernel_proof':proof}}
...
def test_fresh_plan_is_frozen_by_existing_source_collector(monkeypatch):
    ...
    assert str(REPOSITORY/'docs/codex/gesc_gaussian/v2/m4_v3_recorder_shutdown_plan.md') in paths
    assert '/opt/ros/humble/lib/python3.10/site-packages/ros2run/api/__init__.py' in paths


@pytest.mark.parametrize('version', ['m4-pilot-v1', 'm4-pilot-v2'])
def test_v3_preserves_both_prior_population_resource_and_scientific_outputs(version):
    old_scenario=module('m4_prior_scenario_v3_parity', BEFORE/'ros2_ws/src/ros_esc/ros_esc/scenario_runner/m4_scenario.py')
    old_scenario.SCENARIOS=scenario.SCENARIOS
    old_metrics=module('ros_esc.plotting_scripts.m4_prior_metrics_v3_parity', BEFORE/'ros2_ws/src/ros_esc/ros_esc/plotting_scripts/m4_pilot.py')
    old_dispatch=module('m4_prior_dispatch_v3_parity', BEFORE/'docs/codex/gesc_gaussian/v2/tools/run_m4.py')
    assert scenario.experiment_identity(version)==old_scenario.experiment_identity(version)
    assert scenario.expected_slots(version)==old_scenario.expected_slots(version)
    kwargs=dict(runs_root=Path(scenario.experiment_identity(version)['root'])/'runs',experiment_version=version)
    assert scenario.build_m4_document(topology(),**kwargs)==old_scenario.build_m4_document(topology(),**kwargs)
    assert metrics.aggregate_pilot([],experiment_version=version)==old_metrics.aggregate_pilot([],experiment_version=version)
    row=scenario.expected_slots(version)[2]
    kwargs=dict(run_id=scenario.experiment_run_id(row,version),arm='C',partition='development',condition='nominal',experiment_version=version)
    qualification={'qualified':False,'origin_ns':None,'integrity_errors':['fixture']}
    assert metrics.normalize_direction_targets([],qualification,**kwargs)==old_metrics.normalize_direction_targets([],qualification,**kwargs)
    contract=dict(version=version,scenario={'path':'/fixture/scenario.yaml'},execution={'runs_root':'/fixture/runs'})
    planned=dict(case_id=row['case_id'],run_id=kwargs['run_id'],summary_path='/fixture/summary.yaml',visible=True)
    assert dispatch.runner_command(contract,planned)==old_dispatch.runner_command(contract,planned)
    result={'process_ownership':{'mode':'subreaper_v2','inspection_complete':True,'kernel_proof':{
        key:True for key in ('complete','baseline_echild','subreaper_verified','root_reaped','final_echild','scope_exclusive','sigchld_valid','state_restored')}}}
    assert dispatch.validate_process_ownership(contract,result)==old_dispatch.validate_process_ownership(contract,result)


@pytest.mark.parametrize('earlier', ['m4-pilot-v1', 'm4-pilot-v2'])
@pytest.mark.parametrize('fault', ['root', 'case', 'run', 'suite', 'mode', 'experiment', 'analysis'])
def test_v3_cannot_substitute_either_retained_experiment(contract,earlier,fault):
    identity=scenario.experiment_identity(earlier)
    old=scenario.expected_slots(earlier)[0]
    row=contract['runs'][0]
    if fault=='root':contract['root']=identity['root']
    ...
    elif fault=='mode':contract['execution']['process_ownership_mode']=identity['process_ownership_mode']
        ...
        with pytest.raises(ValueError):metrics.validate_experiment_document(document,contract['version'])
        return
    with pytest.raises(ValueError):dispatch.validate_contract(contract)


@pytest.mark.parametrize('strategy', [None, '', 'pidfd_only', 'initial_reaped_root_group_sigint'])
def test_v3_proof_requires_exact_declared_graceful_delivery(contract,strategy):
    result={'process_ownership':proof(999)}
    result['process_ownership']['signal_strategy']=strategy
    with pytest.raises(ValueError,match='signal strategy'):dispatch.validate_process_ownership(contract,result)


@pytest.mark.parametrize('version', ['m4-pilot-v0', 'm4-pilot-v15', 'm4-pilot-v03', 'm4_pilot_v3', None])
def test_v3_remains_an_exact_allowlist(version):
...
def test_v3_acquisition_binding_rejects_metadata_substitution_before_read(contract,fault,monkeypatch):
...
def test_v3_reference_requires_exact_completed_label_contract_before_any_model(contract,fault,monkeypatch):
...
def proof(pid):
    return {'mode':'subreaper_group_v3','signal_strategy':'initial_unreaped_root_group_sigint_then_adopted_pidfd','inspection_complete':True,
...
def test_v3_dispatch_passes_mode_to_inner_outer_and_cleanup_without_extra_slots(contract,monkeypatch):
    ...
    def process(argv,*args,**kwargs):
        assert args==(900.,45.) and kwargs['process_ownership_mode']=='subreaper_group_v3'
        assert kwargs['strict_process_tracking'] is True
        assert argv[argv.index('--process-ownership-mode')+1]=='subreaper_group_v3'
    ...
    def cleanup(*args,**kwargs):
        assert kwargs['strict'] is True and kwargs['process_ownership_mode']=='subreaper_group_v3'
...
def test_v3_science_hooks_propagate_ownership_and_preserve_caps(contract,monkeypatch):
...
def test_v3_summary_cannot_import_another_experiment_contract(contract):
    path=Path(contract['contract_path']);path.parent.mkdir();path.write_text(json.dumps(contract))
    labels=dict(version=metrics.VERSION,**metrics.experiment_fields(contract['version']),
