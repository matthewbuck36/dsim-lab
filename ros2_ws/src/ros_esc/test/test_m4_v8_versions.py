"""Fresh V8 identity with exact V7 controls and retained seven-version parity.
...
RETAINED = Path('/home/mattb/Experiments/GESC-Gaussian/v2/builds/'
                'm4_v8_stationary_causality_comparison_v1/version_routes')
CAPTURE_SHA = '6ea6bde7ae18f8a926ae1dbefffeb6d80ed15807a33d88a20b24338f9d9c9ffb'
EARLIER = tuple(f'm4-pilot-v{value}' for value in range(1, 8))
...
def captured(name):
    ...
    assert receipt['status'] == 'PASS' and receipt['expanded_rows'] == 112
...
def pristine_contract(tmp_path_factory):
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(scenario, 'PILOT_ROOT', tmp_path_factory.mktemp('m4_v8_contract')/'pilot')
...
def test_all_seven_prior_populations_and_112_builder_rows_are_exact(version, monkeypatch):
    ...
    if version in ('m4-pilot-v5', 'm4-pilot-v6', 'm4-pilot-v7'):
...
def test_fresh_v8_identity_keeps_the_default_and_exact_fixed_blocks():
    ...
    assert scenario.experiment_identity(VERSION) == dict(experiment_version=VERSION,
        suite_id='m4_pilot_v8', root='/home/mattb/Experiments/GESC-Gaussian/v2/pilot/m4_pilot_v8',
        process_ownership_mode=MODE)
    prior = scenario.expected_slots('m4-pilot-v7')
    ...
    for old, new in zip(prior, rows):
        assert new == {**old, 'experiment_version': VERSION,
                       'case_id': old['case_id'].replace('m4_v7_', 'm4_v8_', 1)}
        assert new['case_id'] not in previous_ids
        assert scenario.experiment_run_id(new, VERSION) == (
            f'm4-pilot-v8-slot{new["slot"]:02d}-{new["arm"]}-{new["seed"]}')


@pytest.mark.parametrize('value', ['m4-pilot-v15', 'm4-pilot-v08', 'm4_pilot_v8', None])
def test_future_and_noncanonical_versions_remain_rejected(value):
    with pytest.raises(ValueError, match='unsupported'):
        scenario.experiment_identity(value)


def test_v8_inherits_the_complete_v7_controller_and_stage_maps(contract):
    assert scenario.validate_experiment_contract(contract)['experiment_version'] == VERSION
    old = json.loads(captured('m4_pilot_v7_outputs.json').read_text())
...
def test_every_v8_slot_retains_strict_configuration_guards(contract, slot, fault):
...
def test_v8_retains_controller_receipt_guards(contract, fault):
...
def test_v7_receipts_and_identifiers_cannot_be_substituted_into_v8(contract, field):
    row = contract['runs'][0]
    if field == 'root':
        contract['root'] = str(Path(contract['root']).with_name('m4_pilot_v7'))
    elif field == 'run':
        row['run_id'] = row['run_id'].replace('m4-pilot-v8', 'm4-pilot-v7')
    elif field == 'case':
        row['case_id'] = row['case_id'].replace('m4_v8_', 'm4_v7_')
    elif field == 'suite':
        row['resolved_scenario']['suite_id'] = 'm4_pilot_v7'
    elif field == 'summary':
        row['summary_path'] = row['summary_path'].replace('m4_pilot_v8', 'm4_pilot_v7')
    elif field == 'experiment':
        contract['experiment_version'] = 'm4-pilot-v7'
    elif field == 'method':
...
def test_runner_admits_v8_only_with_existing_strict_group_owner(monkeypatch, mode, strict, accepted):
    ...
    monkeypatch.setattr(runner, 'load_suite', lambda *_args: {'suite_id': 'm4_pilot_v8'})
...
def test_method_denominators_and_cross_version_analysis_remain_strict(contract):
    ...
    for version in EARLIER:
        with pytest.raises(ValueError):
            metrics.validate_experiment_document(normalized, version)
