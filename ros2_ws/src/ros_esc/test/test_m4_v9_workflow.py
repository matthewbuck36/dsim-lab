"""V9 workflow selection through existing owners with finite synthetic inputs.

Prepare V8 and V9 once each with numerical/graph operations replaced. Every
...
VERSION = 'm4-pilot-v9'
PREVIOUS = 'm4-pilot-v8'
EARLIER = tuple(f'm4-pilot-v{number}' for number in range(1, 9))
...
def _prepared_versions(tmp_path_factory):
    ...
    directory = tmp_path_factory.mktemp('m4_v9_workflow')
...
def test_preparation_retains_all_sixteen_v8_controls_topologies_and_budgets(
    ...
    assert contract['execution'] == dict(runs_root=str(Path(contract['root'])/'runs'),
        ros_domain_id=201, recording_sec=720., case_timeout_sec=900., suite_timeout_sec=15300.,
    ...
    for index, (old, row) in enumerate(zip(previous['runs'], contract['runs'])):
        ...
        assert row['case_id'] == f'm4_v9_{partition}_{condition}_{arm}_{seed}'
        assert row['run_id'] == f'm4-pilot-v9-slot{index+1:02d}-{arm}-{seed}'
...
def test_prior_version_context_cannot_replace_any_v9_contract_boundary(prepared, earlier, field):
...
@pytest.mark.parametrize('field,value', [('method_version', 'm4-pilot-v9'),
...
def test_source_collector_pins_both_controller_files_and_v9_budget_amendment():
    ...
    expected = {str(runner.GESC_CONTROLLER), str(scenario.V6_CONTROLLER_PATH), str(Path(__file__)),
        ...
        str(workflow.REPOSITORY/'docs/codex/gesc_gaussian/v2/m4_v9_budget_comparison_plan.md')}
...
def test_holdouts_release_once_after_exact_v9_behavior_failure_and_science_timeout(
...
def test_v9_release_refuses_any_started_final_holdout(prepared, development_ledger, opened):
    rows, block = development_ledger
...
def test_v9_release_rejects_wrong_or_incomplete_bound_evidence(
...
def test_v9_public_release_binds_current_contract_preparation_and_source(
...
def test_v9_final_report_retains_all_sixteen_slots_and_192_missing_direction_targets(
        prepared, development_ledger):
    rows, block = development_ledger
...
def test_v9_finalizer_rejects_rehashed_prior_version_acquisition(prepared, monkeypatch, fault):
    monkeypatch.setattr(workflow.time, 'monotonic', lambda: 100.)
    ...
    if fault == 'experiment':
        ...
        record['case_id'] = record['case_id'].replace('m4_v9_', 'm4_v8_', 1)
