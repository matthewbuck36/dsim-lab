"""Fresh V9 identity with exact V8 controls and retained eight-version parity.
...
RETAINED = Path('/home/mattb/Experiments/GESC-Gaussian/v2/builds/'
                'm4_v9_budget_comparison_v1/version_routes')
CAPTURE_SHA = 'a7776dda9a0d6abbefcb8886fabb4be8c676449d3ad259d3c82618857bbca11a'
EARLIER = tuple(f'm4-pilot-v{value}' for value in range(1, 9))
...
def captured(name):
    ...
    assert receipt['status'] == 'PASS' and receipt['expanded_rows'] == 128
...
def pristine_contract(tmp_path_factory):
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(scenario, 'PILOT_ROOT', tmp_path_factory.mktemp('m4_v9_contract')/'pilot')
...
def test_all_eight_prior_populations_and_128_builder_rows_are_exact(version, monkeypatch):
    ...
    if version in ('m4-pilot-v5', 'm4-pilot-v6', 'm4-pilot-v7', 'm4-pilot-v8'):
...
def test_fresh_v9_identity_keeps_the_default_and_exact_fixed_blocks():
    ...
    assert scenario.experiment_identity(VERSION) == dict(experiment_version=VERSION,
        suite_id='m4_pilot_v9', root='/home/mattb/Experiments/GESC-Gaussian/v2/pilot/m4_pilot_v9',
        process_ownership_mode=MODE)
    prior = scenario.expected_slots('m4-pilot-v8')
    ...
    for old, new in zip(prior, rows):
        assert new == {**old, 'experiment_version': VERSION,
                       'case_id': old['case_id'].replace('m4_v8_', 'm4_v9_', 1)}
        assert new['case_id'] not in previous_ids
        assert scenario.experiment_run_id(new, VERSION) == (
            f'm4-pilot-v9-slot{new["slot"]:02d}-{new["arm"]}-{new["seed"]}')


@pytest.mark.parametrize('value', ['m4-pilot-v15', 'm4-pilot-v09', 'm4_pilot_v9', None])
def test_future_and_noncanonical_versions_remain_rejected(value):
    with pytest.raises(ValueError, match='unsupported'):
        scenario.experiment_identity(value)


def test_v9_inherits_the_complete_v8_controller_and_stage_maps(contract):
    assert scenario.validate_experiment_contract(contract)['experiment_version'] == VERSION
    old = json.loads(captured('m4_pilot_v8_outputs.json').read_text())
...
def test_every_v9_slot_retains_strict_configuration_guards(contract, slot, fault):
...
def test_v9_retains_controller_receipt_guards(contract, fault):
...
def test_v8_receipts_and_identifiers_cannot_be_substituted_into_v9(contract, field):
    row = contract['runs'][0]
    if field == 'root':
        contract['root'] = str(Path(contract['root']).with_name('m4_pilot_v8'))
    elif field == 'run':
        row['run_id'] = row['run_id'].replace('m4-pilot-v9', 'm4-pilot-v8')
    elif field == 'case':
        row['case_id'] = row['case_id'].replace('m4_v9_', 'm4_v8_')
    elif field == 'suite':
        row['resolved_scenario']['suite_id'] = 'm4_pilot_v8'
    elif field == 'summary':
        row['summary_path'] = row['summary_path'].replace('m4_pilot_v9', 'm4_pilot_v8')
    elif field == 'experiment':
        contract['experiment_version'] = 'm4-pilot-v8'
    elif field == 'method':
...
def test_runner_admits_v9_only_with_existing_strict_group_owner(monkeypatch, mode, strict, accepted):
    ...
    monkeypatch.setattr(runner, 'load_suite', lambda *_args: {'suite_id': 'm4_pilot_v9'})
