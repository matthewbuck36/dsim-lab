"""This script holds pre-made cost functions
...
class CostFunction(ABC):
    ...
    output method. This method will always receive the same
...
class PositionBasedSympyExpression(CostFunction):
    """This class allows for a position based sympy expression to be used as a cost funciton."""

    def __init__(self, params):
        """This initializes the object.
        
        The sympy expression should be defined as a dictionary named params in the
        following format. The function key should contain the cost function expressed
        symbolically as a string. The symbols key should match the one shown below,
        all cost functions will be functions of position and time. Finally, the substitutions
        key is optional, this can be used to define a dictionary of variables that will get
        substituted into the main expression. These variables may be defined as ints or floats,
        or they may contain more expressions expressed as strings.
...
class PhotoresistorInterpolatedMap(CostFunction):
    """This creates an interpolated map of photoresistor data to be used as a cost function."""

    def __init__(self, params):
        """This initializes the object.
        
        The interpolated map should be defined as a dictionary named params in the
        following format. The mode key is how the user selects if they would like to
        return a resistance, or voltage cost value based off of experimentally collected
        data. This node uses a quadratic cost function based off of the resistance,
        however if the user desires to work with voltage instead, the object will convert
        its final resistance value into a voltage reading before returning the result.
        Note that the voltage reading increases when the resistance falls, thus to ensure
        our optimization problem is a minimization problem, we multiply the voltage value
        by negative one.
