...
"""
Demonstrates using directions from AdaGrad ESC
"""
...
from extremum_seeking.seekers import LieBracketSeeker, AdaGradFlow
...
if __name__ == "__main__":
    ...
    # Sensor to return gradient and gradient outer product
    y = lambda t, x: np.hstack((
        J.gradient(t, x),
        np.outer(J.gradient(t, x), J.gradient(t, x)).flatten('F')
    ))
    ...
    states_0 = np.array([2, 2, 0]) # initial x, y, and theta vehicle states
    ghat_0 = np.array([0, 0]) # initial gradient estimate
    gamma_0 = np.array([1, 0, 0, 1]) # initial gradient outer product estimate
    x0 = seeker.initialize_system(
        states_0,
        np.concatenate((ghat_0, gamma_0))
    )
