...
algorithm graph and Phase 05 recorder/manifest/validator. Only adapters may
differ between simulation and physical operation.

Plan:
- physical topic adapters,
- Vicon and sensor validation,
- source-score calibration,
- lamp level/position metadata,
- low-speed limits,
- emergency stop,
- bag preflight,
- one-source progression,
- two-source progression,
- operator checklist,
- rollback to legacy profile.
...
Do not edit source code or command physical hardware in Plan mode. Make the
final Plan response complete enough to save without relying on the chat
history. Before opening the Phase 09 Implement chat, save that response at the
exact path above.
